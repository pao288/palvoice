...
class SetupCog(commands.Cog):
    ...
    async def voice(self, interaction: discord.Interaction):
        ...
        info_embed_msg = main_embed(
            "🎤 PAL VOICE",
            "「🎤 VCを予約する」ボタンから、部屋タイプ・鍵の有無・利用時間などを\n"
            "すべて決めてからVCを作成できます。\n"
            "作成後、ご自身でボイスチャンネルに参加してください。"
        )
        await voice_text_channel.send(embed=info_embed_msg, view=BookingPanelView())
        ...
        await interaction.followup.send(
            embed=success_embed(
                "セットアップ完了",
                f"{voice_text_channel.mention} / {admin_channel.mention} / {log_channel.mention}"
            ),
            ephemeral=True,
        )
