...
class CheckoutView(discord.ui.View):
    ...
    async def _checkout(self, interaction: discord.Interaction):
        ...
        if row is None:
            await interaction.response.send_message(embed=error_embed("エラー", "このVC情報が見つかりませんでした。"), ephemeral=True)
            return
        ...
        if not (is_owner or is_admin_user):
            await interaction.response.send_message(embed=error_embed("権限エラー", "チェックアウトできるのは作成者か管理者のみです。"), ephemeral=True)
            return
        ...
        async def do_checkout(confirm_interaction: discord.Interaction):
            ...
            if latest is None:
                await confirm_interaction.response.edit_message(embed=error_embed("エラー", "既に終了しています。"), view=None)
                return
